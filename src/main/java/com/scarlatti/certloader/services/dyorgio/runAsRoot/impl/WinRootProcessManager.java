 ...
 * ****************************************************************************
 */
package com.scarlatti.certloader.services.dyorgio.runAsRoot.impl;

import com.scarlatti.certloader.services.dyorgio.runAsRoot.NotAuthorizedException;
import com.scarlatti.certloader.services.dyorgio.runAsRoot.RootProcessManager;
...
//            + ":--------------------------------------\n";
    ...
    static {
        try {
            COMMAND_TO_ADMIN = new Scanner(WinRootProcessManager.class.getResourceAsStream("/runner.bat")).useDelimiter("\\Z").next();
//            VBSCRIPT_TO_ADMIN = new String(Files.readAllBytes(Paths.get(WinRootProcessManager.class.getResource("/runner.vbs").toURI())));
    ...
    public ProcessBuilder create(List<String> commands) throws Exception {
        ...
        String commaSeparatedArgsList = StringUtils.join(commands, ", ");

        builder.append(COMMAND_TO_ADMIN.replace("COMMA_SEPARATED_ARGS_LIST", commaSeparatedArgsList));
